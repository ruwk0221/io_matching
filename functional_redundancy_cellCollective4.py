            ...
            for s in Sources:
                for t in Targets:
                    if nx.has_path(G=GRead, source=InverseReadNodes[s.replace('~', '')], target=InverseReadNodes[t.replace('~', '')]):
                        ...
                        for p in nx.all_simple_paths(G=GRead, source=InverseReadNodes[s.replace('~', '')], target=InverseReadNodes[t.replace('~', '')], cutoff=2*len_shortest_path):
                            io_pathway_count += 1
...
with open('io_pathway_counts_cellCollective5.csv', 'w') as f:
    writer = csv.writer(f)
